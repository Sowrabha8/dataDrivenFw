...
public class JenkinsTest extends BaseTest{
	...
	public void createJob() {
		 ...
		 * driver.findElement(By.xpath("//li[contains(@class,'FreeStyleProject')]")).
		 * click(); wait = new WebDriverWait(driver, 100);
		 * wait.until(ExpectedConditions.elementToBeClickable(By.id("ok-button")));
		 * driver.findElement(By.id("ok-button")).click();
		 * wait.until(ExpectedConditions.elementToBeClickable(By.xpath(
		 * "//button[text()='Apply']"))); ((JavascriptExecutor)
		 * driver).executeScript("window.scrollTo(0, document.body.scrollHeight)");
		 * driver.findElement(By.xpath("//button[text()='Add build step']")).click();
		 * wait.until(ExpectedConditions.visibilityOf(driver.findElement(By.
		 * xpath("//div[contains(@class,'menu-button-menu visible')]//ul[@class='first-of-type']"
		 * ))));
		 * driver.findElement(By.linkText("Execute Windows batch command")).click();
		 * wait.until(ExpectedConditions.elementToBeClickable(By.xpath(
		 * "//div[@name='builder']/table/tbody//div[@class='advancedLink']//button")));
		 * driver.findElement(By.name("command")).sendKeys("echo hello");
		 * driver.findElement(By.xpath("//button[text()='Save']")).click();
		 * wait.until(ExpectedConditions.elementToBeClickable(By.
		 * xpath("//button[text()='Disable Project']")));
		 * wait.until(ExpectedConditions.elementToBeClickable(By.
		 * linkText("RSS for failures")));
